...
REMAINING_OPEN_GATES: set[str] = set()
...
def test_audit_storage_contract_closes_only_g7_and_remains_no_go() -> None:
    ...
    gates = status["gates"]
    assert set(gates) == {"G1", "G2", "G3", "G4", "G5", "G6", "G7", "G8"}
    ...
    assert gates["G8"]["status"] == "closed"
    assert gates["G8"]["closed_by_task"] == "175"
    assert gates["G5"]["status"] == "closed"
    assert gates["G5"]["closed_by_task"] == "176"
    assert gates["G5"]["decision"] == "NO-GO"
