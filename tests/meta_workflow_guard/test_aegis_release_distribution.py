...
def test_mcp_client_setup_doc_covers_cross_agent_release_candidate_configs() -> None:
    ...
    for snippet in (
        ...
        "native client tool/resource/prompt discovery",
    ):
        assert snippet in text
...
def test_ci_templates_and_release_matrix_cover_distribution_dimensions() -> None:
    ...
    for snippet in (
        ...
        "`aegis --version`, `aegis inspect`, `aegis status`, `aegis next`, `aegis plan-install`, `aegis install --apply`, `aegis verify`, `aegis verify --strict`, `aegis closeout --dry-run`, `aegis certify-release`, `aegis start`, `aegis kickoff`, `aegis log`",
        ...
        "`aegis mcp smoke-registration` proves native Codex and Claude registration in isolated temporary homes/config directories",
        "missing native MCP clients return structured `missing_client` evidence without writing fallback config files",
        "manual `.mcp.json` and Codex config-file writes are fallback-only",
...
def test_live_acceptance_matrix_and_adapter_contract_are_packaged() -> None:
    ...
    for snippet in (
        ...
        "Codex | implemented passive-evidence adapter",
