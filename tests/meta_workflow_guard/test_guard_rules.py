...
def test_validate_work_tracking_folder_names_allows_untracked_multi_day_started_folder(
    monkeypatch,
) -> None:
    module = load_guard_module()
    monkeypatch.setattr(module, "TODAY_ISO", "2030-01-02")
...
def test_validate_session_allows_latest_completed(monkeypatch) -> None:
    ...
    latest = (
        module.REPO_ROOT / "sessions" / "2025" / "11" / "2025-11-24-001-task89-work-tracking.md"
    )
...
def test_validate_session_allows_multiple_completed_sessions_from_latest_prior_date(
    monkeypatch,
) -> None:
    module = load_guard_module()
...
def test_validate_session_flags_older_completed(monkeypatch) -> None:
    ...
    monkeypatch.setattr(module, "TODAY_ISO", "2025-11-25")
    older = (
        module.REPO_ROOT / "sessions" / "2025" / "10" / "2025-10-21-001-task88-alignment-docs.md"
    )
...
def test_validate_session_state_flags_missing_paused_target(monkeypatch, tmp_path) -> None:
    ...
    issues = module.validate_session_state(entries, changed)
    assert any(
        "Paused session '2026-04-22-001-task91-continuation.md' does not exist" in issue.message
        for issue in issues
    )
...
def test_between_sessions_state_rejects_missing_symlink_with_active_session(
        ...
        json.dumps(
            {
                "status": "completed",
                "paths": {
                    "work_tracking": completed_tracker.parent.relative_to(_repo_root).as_posix()
                },
            }
        ),
        encoding="utf-8",
...
def test_get_active_tracker_path_rejects_non_completed_current_work(monkeypatch, tmp_path) -> None:
    module = load_guard_module()
    repo_root, _active_root, archive_root, current_work = _configure_tracker_resolution_fixture(
        module, monkeypatch, tmp_path
    )
...
def test_collect_validation_issues_includes_archived_tracker(monkeypatch, tmp_path) -> None:
    module = load_guard_module()
...
def test_validate_taskmaster_activity_requires_session_and_tracker_entries(
    monkeypatch, tmp_path
) -> None:
    module = load_guard_module()
    monkeypatch.setattr(module, "TODAY_ISO", "2030-01-02")
...
def test_build_tracker_last_updated_fix_updates_stale_date(monkeypatch) -> None:
    ...
    monkeypatch.setattr(module, "TODAY_ISO", "2030-01-02")
    relative = Path(
        "docs/ai/work-tracking/active/20300101-task39-guard-auto-fix-mode-ACTIVE/TRACKER.md"
    )
    text = """# Tracker
...
def test_guard_validate_preview_does_not_apply_fixes(monkeypatch, capsys) -> None:
    module = load_guard_module()
    issue = module.ValidationIssue(
        Path("docs/ai/work-tracking/active/example/TRACKER.md"), "Tracker Last Updated is stale"
    )
    fix = module.GuardFix(
        kind="tracker-last-updated",
    ...
    monkeypatch.setattr(
        ...
        lambda include_untracked: ([issue], [module.REPO_ROOT / fix.path]),
    )
    monkeypatch.setattr(
        module, "collect_auto_fixes", lambda changed_files, selected_kinds=None: [fix]
    )
...
def test_guard_validate_reports_remaining_failures_after_auto_fix(
    monkeypatch, tmp_path, capsys
) -> None:
    module = load_guard_module()
    initial_issue = module.ValidationIssue(Path("TRACKER.md"), "Tracker Last Updated is stale")
    remaining_issue = module.ValidationIssue(
        Path("plans/current"), "Active plan symlink missing or invalid"
    )
    fix = module.GuardFix(
        kind="tracker-last-updated",
        path=Path("docs/ai/work-tracking/active/example/TRACKER.md"),
    ...
    calls = iter(
        [
            ([initial_issue], [module.REPO_ROOT / fix.path]),
            ([remaining_issue], [module.REPO_ROOT / fix.path]),
        ]
    )
    ...
    monkeypatch.setattr(
        module, "apply_auto_fixes", lambda fixes, history_path: applied.extend(fixes)
    )
...
def test_validate_gac_guidance_ignores_deprecated_compaction_detection() -> None:
    module = load_guard_module()
    text = (
        module.REPO_ROOT / "templates" / "behaviors" / "session" / "compaction-detection.md"
    ).read_text(encoding="utf-8")
    issues = module.validate_gac_guidance(
        Path("templates/behaviors/session/compaction-detection.md"), text
    )
    assert issues == []
...
def test_deprecated_compaction_detection_not_canonical_gac_doc() -> None:
    module = load_guard_module()
    assert (
        Path("templates/behaviors/session/compaction-detection.md") not in module.GAC_SUMMARY_DOCS
    )
...
def test_detect_tracked_active_deletions_flags(monkeypatch) -> None:
    ...
    entries = [
        (
            " D",
            "docs/ai/work-tracking/active/20251021-task88-taskmaster-alignment-ACTIVE/TRACKER.md",
        )
    ]
    monkeypatch.setattr(module, "_path_tracked", lambda rel: True)
    monkeypatch.setattr(
        ...
        module.REPO_ROOT / "docs" / "ai" / "work-tracking" / "archive-test",
    )
...
- **2030-01-01 10:00** — [S:20300101|W:task99-workflow|H:serena/memory|E:memories/2030-01-01_task99] Logged progress
        ...
        (folder / "CHANGELOG.md").write_text(
            "- 2030-01-01 10:01 CET — Changelog updated\n", encoding="utf-8"
        )
        issues = module.validate_work_tracking_documents(folder)
...
def test_detect_tracked_active_deletions_flags_when_not_archived(monkeypatch) -> None:
    ...
    active_folder = (
        ...
        / "work-tracking"
        / "active"
        / "20300101-task99-workflow-ACTIVE"
    )
    archive_folder = (
        module.REPO_ROOT
        / "docs"
        ...
        / "work-tracking"
        / "archive"
        / "20300101-task99-workflow-COMPLETED"
    )
    try:
        ...
        monkeypatch.setattr(
            module,
            "_path_tracked",
            lambda rel: rel.as_posix() == tracker_path.relative_to(module.REPO_ROOT).as_posix(),
        )
...
def test_detect_tracked_active_deletions_ignored_when_archived(monkeypatch) -> None:
    ...
    active_folder = (
        ...
        / "work-tracking"
        / "active"
        / "20300102-task99-workflow-ACTIVE"
    )
    archive_folder = (
        module.REPO_ROOT
        / "docs"
        ...
        / "work-tracking"
        / "archive"
        / "20300102-task99-workflow-COMPLETED"
    )
    try:
        active_folder.mkdir(parents=True, exist_ok=True)
        tracker_path = active_folder / "TRACKER.md"
...
def test_select_template_metadata_rule_matches_handlers() -> None:
    module = load_guard_module()
    rule = module.select_template_metadata_rule(
        Path("templates/handlers/orchestrators/session-start.md")
    )
...
def test_select_template_metadata_rule_matches_matrices() -> None:
    module = load_guard_module()
    rule = module.select_template_metadata_rule(
        Path("templates/matrices/routing/request-to-handler.md")
    )
    assert rule is not None
...
def test_select_template_metadata_rule_matches_shared_core_pattern() -> None:
    module = load_guard_module()
    rule = module.select_template_metadata_rule(
        Path("templates/shared/patterns/ultrathink-format.md")
    )
    assert rule is not None
...
def test_select_template_metadata_rule_matches_pattern_templates() -> None:
    module = load_guard_module()
    rule = module.select_template_metadata_rule(
        Path("templates/patterns/session/session-patterns.md")
    )
    assert rule is not None
...
def test_validate_template_metadata_flags_missing_frontmatter(monkeypatch) -> None:
    ...
    monkeypatch.setattr(module, "select_template_metadata_rule", lambda path: fake_rule)
    issues = module.validate_template_metadata(
        Path("templates/handlers/example.md"), "# No frontmatter\n"
    )
...
def test_collect_template_metadata_drift_supports_cross_project_template_roots(
    monkeypatch, tmp_path
) -> None:
    module = load_guard_module()
...
def test_collect_canonical_doc_drift_flags_missing_doc(monkeypatch) -> None:
    ...
    assert any(
        finding.path == missing and "missing" in finding.message.lower() for finding in findings
    )
