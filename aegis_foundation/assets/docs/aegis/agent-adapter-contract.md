...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Claude | implemented default | `CLAUDE.md`, `.claude/settings.json`, `.claude/scripts/*.sh` | PreToolUse, PostToolUse, Stop hooks for supported tools | `claude mcp add --scope user/project aegis ...` or project `.mcp.json` fallback | Edit, Write, MultiEdit, NotebookEdit, Bash, Agent, Task, MCP calls routed through Claude Code | mechanical for hookable mutations and managed-project native-delegation denial; policy-only for unsupported memory/MCP surfaces | S:W:H:E entries in session, tracker, implementation, changelog, handoff, and plan evidence | readiness, pending-tracking block, protected path guard, managed-delegation regressions, strict verify, closeout |
| Codex | implemented managed adapter | `CODEX.md`, `.codex/hooks.json`, shared `.claude/scripts/*` dispatch runtime | PreToolUse, PostToolUse, SessionStart, Stop, SubagentStart, and SubagentStop command hooks; canonical `apply_patch` is parsed directly | `codex mcp add ... aegis ...` | Bash, canonical `apply_patch`, provider-native create/resume tools, MCP calls, and supported lifecycle events | mechanical for trusted PreToolUse definitions; SubagentStart/Stop remain passive lifecycle recording; client-owned exact-hash trust remains attended | one atomic patch event with all affected paths in the shared out-of-worktree ledger, hierarchical attribution where supported, and complementary S:W:H:E surfaces | managed install parity, exact-hash `/hooks` review, managed-delegation regressions, canonical payload regressions, linked-worktree/child capture, strict verify, and branch-safe witness/replay |
...
For Gas City managed projects, provider-native work creation/resumption is a tier-c PreToolUse
decision and is denied even in advisory mode. Claude `Agent`/`Task` and Codex `spawn_agent`,
`assign_agent_task`, `followup_task`, and `resume_agent` (including collaboration-qualified names)
must not replace a Bead plus reviewed `gc sling` route. A failed route remains a stop condition.
`SubagentStart` cannot supply this guarantee and remains passive evidence. The optional
`.gas-city-delegation-exceptions.json` escape hatch is not caller-based: every record binds exact
project, adapter, normalized tool, request digest, `codex/*` branch, Bead, and review evidence;
the whole file must be clean at HEAD and byte-identical on the project's canonical reviewed base
(`base_ref`, normalized to its origin-tracking ref, or `origin/main`). Invalid or unmatched records fail closed and ordinary Aegis break-glass tokens do not
apply.
