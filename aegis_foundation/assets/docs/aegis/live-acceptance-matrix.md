...
This matrix defines the live evidence needed before Aegis is treated as a portable workflow runtime, not just an installable package. The goal is to prove the same workflow behavior across fresh and already-started projects, both without optional integrations and with Taskmaster when a project already uses it.

Public-flow rows must exercise `aegis init` and `aegis start "<normal task title>"` where no external task id exists. MCP clients should reach the same standalone behavior through `aegis.init apply=true` followed by `aegis.start apply=true`. Taskmaster-backed rows must exercise `task-master next/show` plus explicit `aegis kickoff --task <id> ...` or MCP `aegis.kickoff apply=true`, because Taskmaster is the task authority in those projects. Existing low-level plan-install/install/kickoff rows remain useful for regression coverage, but public acceptance is measured against the normal command path and a no-large-prompt Claude session.
...
- for Taskmaster-backed rows, proof that Taskmaster `done` is set only after Aegis closeout and read-only doctor pass
- for first-time Claude installs, proof that `client_reload.required=true` causes a stop/restart before source edits and that post-restart edits produce pending tracking
...
| web-taskmaster | Fresh or copied web app with `.taskmaster/tasks/tasks.json` and one available numeric task | MCP `aegis.init`, Taskmaster CLI/MCP for next/show/done, MCP `aegis.kickoff`, native Claude edit | inspect/next -> init -> Claude restart if client_reload required -> task-master next/show -> kickoff with Taskmaster id -> scope log -> native edit -> pending log -> app verify -> strict verify -> closeout_ready -> closeout -> doctor -> task-master done -> targeted task file refresh or deliberate `task-master generate` | Taskmaster task file, `.aegis/reports/install-report.json`, current-work with Taskmaster id, session/plan/work-tracking scaffold, changed source, task verification report, strict verify report, closeout report, doctor result, Taskmaster refresh evidence | no local Aegis task is allocated; all workflow surfaces use the Taskmaster id; pending queue is empty; Taskmaster completion happens after closeout and doctor | Task 131 automated plus manual-live |
