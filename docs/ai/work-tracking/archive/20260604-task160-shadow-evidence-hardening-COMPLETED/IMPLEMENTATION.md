...
- Treat state.json value-lock presence transitions as meaningful mutations: active-tag injection, clearing, removal, and branch mapping removal now refuse instead of passing through absent/empty asymmetry.
...
- Run the CI accumulation step with `PYTHONDONTWRITEBYTECODE=1` so the strict zero-delta oracle is not made brittle by bytecode writes.
...
- Follow-up focused: `PYTHONDONTWRITEBYTECODE=1 python3 -m pytest tests/meta_workflow_guard/test_aegis_reconcile_shadow_apply.py tests/meta_workflow_guard/test_aegis_installer.py tests/meta_workflow_guard/test_ci_workflows.py` — 126 passed, 1 skipped.
- Follow-up full: `PYTHONDONTWRITEBYTECODE=1 python3 -m pytest` — 1092 passed, 4 skipped.
