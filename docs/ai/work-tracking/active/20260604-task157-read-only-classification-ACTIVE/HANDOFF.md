...
## Current State
- Task 157 implementation is complete in the working tree.
- The live hook and packaged hook both include Aegis `target_dir` confinement before read-only short-circuit, and degraded fallback now reuses the same low-level Bash/MCP read-only decisions as the main path.
