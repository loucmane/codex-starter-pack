...
- Meta workflow enforcement active: workflow edits trigger guard check requiring plan scope to include meta workflow orchestrator/pattern and matching plan scope.
- CI/pre-commit plan drafted (pre-commit hook + GitHub Actions job) to run `scripts/codex-guard`, and `codex-task` now prints remediation guidance when guard-related errors occur.
