...
- 2026-05-16 — `aegis://` resources return JSON envelopes instead of raw file contents so missing target state can be represented as structured `not_installed` or `not_available` payloads without side effects. Prompt output remains advisory and is never treated as verification evidence.
- 2026-05-16 — The project-local MCP config registers Aegis as a direct stdio server via `python3 scripts/aegis-mcp-server`. This keeps the MCP server independent from `scripts/codex-task` while preserving Taskmaster and Serena MCP entries.
