...
| --- | --- | --- | --- | --- | --- | --- |
| web-new | Fresh Vite-style web app with no Aegis, no Taskmaster, no Serena | MCP install, native Claude edit, MCP/CLI log | inspect -> plan_install -> install -> kickoff -> scope log -> native edit -> pending log -> app verify -> strict verify -> closeout_ready -> closeout | `.aegis/reports/install-report.json`, `src/main.ts`, task verification report, `.aegis/reports/verification-report.json`, `.aegis/reports/closeout-report.json` | all six workflow surfaces reference the changed source file and pending queue is empty | automated plus manual-live |
| web-started | Existing web app with source files and package scripts | MCP install into existing project | same as web-new, with no overwrite of unrelated files | install plan conflict list, changed app source, npm or project verification evidence | closeout passes without hand-editing `.aegis/` | automated plus manual-live |
| python-new | Fresh Python package or script project | CLI or MCP install | install -> kickoff -> scope -> native Python edit -> pytest/command verify -> strict verify -> closeout | Python source evidence, pytest or command output report | plan-step-scope, implement, verify completed through `plan_step=auto` | automated |
| python-started | Existing Python project | CLI or MCP install | inspect existing files -> safe install -> task flow | install report, source diff, test evidence | no Taskmaster/Serena dependency unless explicitly required | automated |
| backend-new | Fresh backend/server skeleton | CLI or MCP install | same as python-new, with server route or handler edit | route/handler source, smoke command report | closeout-ready dry-run reports pass before final closeout | automated |
| backend-started | Existing backend service | CLI or MCP install | inspect -> plan_install -> install -> task flow | conflict review, source edit, service-level verification | closeout report and handoff cite implementation and verification evidence | automated |
