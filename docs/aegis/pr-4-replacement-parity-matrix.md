...
Every workflow responsibility remains `keep` or `shadow`. Task 210 is **NO-GO**.
Bead `ga-iqbd` separately proves that two duplicated Claude
implementation surfaces may be `demote`: they are compatibility launchers for the
same retained strict boundary, not retirement of the boundary itself.
...
| `.claude/scripts/readiness.sh` implementation | Historically embedded the complete readiness evaluator inside a Claude-named shell script. | Claude compatibility adapter. | `aegis_foundation/gate/{models,state,workflow,render,readiness}.py` plus `aegis gate readiness`; the shell path is a policy-free compatibility launcher. | Differential ready/blocked output and exit-code parity; source-closeout, bead, Taskmaster-compatibility, observation, pointer, and plan/tracker tests; installed dispatcher fallback; rollback to the prior managed asset. | Tasks 237–252 provide the readiness corpus; bead ga-iqbd runs the source, adapter, closeout, installer, Blog-like, and HP-Fetcher-like suites against the canonical evaluator. | Only the historical command path during a migration window; it owns no readiness policy after ga-iqbd. | Restore the previous managed readiness asset and runtime pointer; no workflow state migration is involved. | `demote` | GO — implementation demoted to a compatibility launcher; strict readiness responsibility remains `shadow`. |
| `.claude/scripts/gate_lib.py` implementation | Historically combined payload parsing, shell grammar, hard policy, decisions, tracking, lifecycle, and dispatch in one Claude-owned file. | Claude compatibility adapter. | `aegis_foundation/gate/hooks/` domain modules plus a fail-closed 44-line compatibility loader shared by Claude and Codex. | Existing adversarial hook corpus remains green; module dependency/size contract, strict/advisory parity, break-glass, degraded-mode, delivery, tracking, and cross-project install/update tests pass. | Tasks 237–252 and Blog/HP-Fetcher fixtures preserve the accumulated hook corpus; ga-iqbd replays it through the modular runtime. | Only target-local bootstrap and historical import compatibility; substantive policy has one package implementation. | Restore the prior managed gate asset and runtime pointer; decision and ledger evidence remain untouched. | `demote` | GO — monolith demoted to a compatibility loader; every policy responsibility remains enforced. |
...
- every responsibility state is `keep` or `shadow`;
- only the two ga-iqbd compatibility implementation rows are `demote`/`GO`;
- every broad Task 210 decision remains `NO-GO`;
- the matrix cites Task 243 cross-repository evidence;
- Task 210 depends on TM-229, TM-233, and TM-243.

Any other future move to `demote` or `retire` must deliberately revise this contract
and carry new per-surface evidence. It cannot happen as an incidental cleanup.
