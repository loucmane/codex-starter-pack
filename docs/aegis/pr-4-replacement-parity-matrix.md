...
PR-4 also MUST NOT assume that retirement means deletion. The preferred transition for
human-readable legacy surfaces is often `demote`: the old surface remains as a generated or
view-only S:W:H:E projection from the passive ledger while capsule, Taskmaster, git, and witness
own current truth. See `docs/aegis/legacy-shadow-sweh-projection-contract.md`.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `sessions/` markdown logs | Preserve chronological session history, command/evidence notes, failures, and recovery context. | Session/log scaffolding plus old closeout evidence gates. | Out-of-worktree append-only ledger + generated S:W:H:E session projection + computed capsule summaries. | Fresh agent can answer what happened and what failed from ledger/capsule/projection without chat transcript for several real tasks; projection is idempotent and preserves human prose. | At least three resumed tasks where capsule/ledger/projection reconstructs session history with fewer or equal misses than `sessions/`. | Re-enable session markdown generation and make closeout read session files again. | `shadow` | NO-GO for retirement; continue shadow comparison and TM-233 projection. |
...
| `plans/` markdown plans | Preserve declared intent, plan steps, progress, and verification expectations. | Plan scaffolding + old plan sync/closeout checks. | Scope records + Taskmaster task details + generated plan projection + witness scope/diff accounting. | Witness can prove shipped diff matches declared scope without a hand-maintained plan file, and generated plan projection preserves useful intent/progress context. | Several PRs where witness scope accounting passes and no required intent only exists in human-maintained `plans/`. | Keep plans as required and restore plan-sync checks. | `shadow` | NO-GO until scope records cover non-task branches and ambiguous work; TM-233 must define projection semantics. |
...
| `TRACKER.md` | Human-readable progress ledger for work-tracking envelopes. | Work-tracking scaffolding. | Ledger event stream + generated S:W:H:E tracker projection + capsule open-loop/task-truth sections. | Progress, blocker, and verification facts can be reconstructed from ledger/capsule/projection with less ceremony; projection never overwrites human-authored history. | At least three completed tasks where generated tracker projection plus capsule covers recovery facts and flags any human-only facts. | Continue generating and requiring `TRACKER.md`. | `shadow` | NO-GO until TM-233 projection and unique-content comparison pass. |
| `HANDOFF.md` | Summarize state for a future agent or resumed session. | Closeout/handoff repair. | Computed capsule injection + generated handoff projection + optional PR-3 narration if needed. | Fresh/resumed agent orients from capsule/projection as well or better than handoff; missing "why" cases are either rare or covered by PR-3. | Real resume/compact evidence: stale assumptions corrected, next action validated, and no critical handoff-only facts. | Restore handoff generation and semantic closeout gates. | `shadow` | NO-GO until resume-drift evidence and TM-233 projection are strong enough. |
...
- HP-Fetcher advisory/capsule evidence from 2026-07-09 is an active TM-233 and PR-4 fixture;
  see `docs/aegis/hpfetcher-capsule-advisory-dogfood-2026-07-09.md`. It shows useful capsule
  resume-drift refresh, large advisory gate-decision volume, stale verification evidence, and
  advisory pending accumulation. Use it to validate generated S:W:H:E projection and legacy
  shadow-check policy before demoting old workflow surfaces.
- The clean-install blog dogfood is the first applied TM-233 projection fixture; see
  `docs/aegis/blog-legacy-shadow-sweh-dogfood-2026-07-09.md`. It projected two passive scope
