...
Use it when preparing or reviewing knowledge-transfer readiness:

```bash
python3 scripts/codex-task knowledge transfer-review \
  --label <label> \
  --report-file <knowledge-transfer-review.json> \
  --runbook-file <knowledge-transfer-review.md>
```

The command reports ready, needs-evidence, or needs-implementation status per domain, lists refresh commands for missing evidence, and includes continuity guidance for using guide hubs, training, communication templates, session lifecycle, work-tracking handoffs, and Serena memories. It does not create hosted knowledge-base software, produce videos, run Q&A systems, create analytics dashboards, maintain contact databases, send notifications, mutate existing evidence sources, or contact external systems.
